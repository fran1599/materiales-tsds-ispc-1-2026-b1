...
function recurrenceEnd(s){return s.until||DATA.collection?.academicYearEnd||null;}
function nextClass(s,from=today()){
 const base=from<s.validFrom?s.validFrom:from;
 const delta=(s.weekday-new Date(base+'T12:00:00Z').getUTCDay()+7)%7;
 const value=shiftDate(base,delta),end=recurrenceEnd(s);
 return end&&value>end?null:value;
}
function calendarURL(s,id=null,from=today()){
 const value=nextClass(s,from);if(!value)return null;
 const stamp=(day,time)=>new Date(day+'T'+time+':00-03:00').toISOString().replace(/[-:]/g,'').replace('.000','');
 const end=recurrenceEnd(s), title=id?subject(id).title:s.subjects.map(x=>subject(x).title).join(' y ');
 let rule='RRULE:FREQ=WEEKLY;BYDAY='+['SU','MO','TU','WE','TH','FR','SA'][s.weekday];
 if(end)rule+=';UNTIL='+stamp(end,'23:59');
 const details=['Meet: '+s.meetUrl,...(s.classroomUrl?['Aula virtual: '+s.classroomUrl]:[]),'Horario de Argentina (UTC−3).',s.subjects.length>1?'Encuentro compartido: '+s.subjects.map(x=>subject(x).title).join(' y ')+'. Agregalo una sola vez.':'',end?'Repetición hasta '+end+' según el registro disponible.':'Sin fecha de fin confirmada. Ajustá el cierre de la serie cuando la materia lo informe.',s.source,'Los cambios del sitio no actualizan automáticamente este evento.'].filter(Boolean).join('\n');
 const params=new URLSearchParams({action:'TEMPLATE',text:title,dates:stamp(value,s.startTime)+'/'+stamp(value,s.endTime),ctz:s.timeZone,details,location:s.meetUrl,recur:rule});
 return 'https://calendar.google.com/calendar/render?'+params.toString();
}
function calendarLink(s,id=null,from=today()){
 const url=calendarURL(s,id,from);
 return url?'<a class="calendar-link" href="'+esc(url)+'" target="_blank" rel="noopener noreferrer" aria-label="Agregar '+esc(id?subject(id).title:s.title)+' a Google Calendar, con repetición semanal" title="Serie semanal desde '+esc(nextClass(s,from))+'">+ Calendar</a>':'<span class="quiet">Período finalizado</span>';
}
function quickAccess(id,compact=false){
 const list=(DATA.schedules||[]).filter(s=>s.subjects.includes(id));
 if(!list.length)return '<p class="quick-missing">Horario y enlace de Meet por confirmar.</p>';
 return '<div class="quick-access'+(compact?' compact':'')+'" aria-label="Horarios y accesos de la materia">'+list.map(s=>'<div class="quick-session"><div class="quick-time"><strong>'+weekdays[s.weekday]+' · '+esc(s.startTime)+'–'+esc(s.endTime)+'</strong><span>Hora de Argentina</span></div><div class="quick-actions"><a class="meet-link" href="'+esc(s.meetUrl)+'" target="_blank" rel="noopener noreferrer" aria-label="Entrar al Meet de '+esc(subject(id).title)+'">Entrar a Meet ↗</a>'+calendarLink(s,id)+(s.classroomUrl?'<a class="classroom-link" href="'+esc(s.classroomUrl)+'" target="_blank" rel="noopener noreferrer">Aula virtual ↗</a>':'')+'</div><p class="recurrence-note">'+(recurrenceEnd(s)?'Serie semanal hasta '+date(recurrenceEnd(s))+' de '+recurrenceEnd(s).slice(0,4)+'.':'Serie semanal sin fecha de fin confirmada.')+'</p></div>').join('')+'</div>';
}
function card(s){
 const n=count(s.id);
 return '<article class="subject-card '+s.color+(n?'':' empty')+'"><details class="card-details"><summary><span class="subject-code" aria-hidden="true">'+esc(s.code)+'</span><h3>'+esc(s.title)+'</h3><p>'+esc(s.period)+'</p><span class="card-disclosure">Horarios y accesos <span aria-hidden="true">⌄</span></span></summary><div class="card-expanded">'+quickAccess(s.id,true)+'<p class="teacher-info"><strong>Docentes y autorías identificadas</strong>'+esc(s.teachers)+'</p></div></details><div class="subject-card-foot"><span>'+(n?n+' materiales':'Sin materiales aún')+'</span><a class="card-go" href="#materia/'+s.id+'" aria-label="Ver materia: '+esc(s.title)+'">Ver materia →</a></div></article>';
}
function home(){
...
function alternateLinks(r){
 return (r.alternateLinks||[]).filter(a=>safeURL(a.url)).map(a=>'<p class="alternate-source"><a class="inline-link" href="'+esc(a.url)+'" target="_blank" rel="noopener noreferrer">'+esc(a.label)+' ↗</a><br><small>'+esc(a.note)+'</small></p>').join('');
}
function row(r,showSubject){
 return '<article class="resource-row"><span class="file-icon" aria-hidden="true">'+esc(r.format)+'</span><div class="resource-content"><button class="resource-title" data-resource="'+r.id+'">'+esc(r.title)+'</button>'+(showSubject?'<div class="quiet" style="font-size:12px;margin-top:4px">'+r.subjects.map(s=>esc(subject(s).short)).join(' · ')+'</div>':'')+'<p>'+esc(r.description)+'</p>'+tags(r)+alternateLinks(r)+'</div><div class="resource-actions"><button class="detail-link" data-resource="'+r.id+'">Ver ficha</button><a class="open-link" href="'+esc(r.url)+'" target="_blank" rel="noopener noreferrer" aria-label="Abrir '+esc(r.title)+' en Drive">Abrir <span aria-hidden="true">↗</span></a></div></article>';
}
function results(list,showSubject=false){
 if(!list.length)return '<div class="empty-state" role="status"><h2>No encontramos ese material.</h2><p>Probá con otro tema o quitá algún filtro. El catálogo sólo incluye los archivos revisados hasta el 19 de septiembre.</p><button class="button secondary" data-reset>Limpiar búsqueda y filtros</button></div>';
...
function schedulesOn(value,id=null){
 ...
 return (DATA.schedules||[]).filter(s=>(!id||s.subjects.includes(id))&&s.weekday===weekday&&value>=s.validFrom&&(!recurrenceEnd(s)||value<=recurrenceEnd(s))).sort((a,b)=>a.startTime.localeCompare(b.startTime));
}
...
function project(){
 $('#main').innerHTML=heading('PROYECTO ESTUDIANTIL','Tecnología, aportes y licencias','Cómo funciona este espacio y cómo podés ayudar a mejorarlo.')+"<div class=\"prose\"><h2>Con qué está hecho</h2><p>HTML estructura las páginas, CSS define el diseño y JavaScript nativo resuelve la navegación, la búsqueda y el calendario. Las materias, descripciones, fechas y enlaces se guardan en un archivo JSON. No usamos un framework ni una base de datos.</p><p>GitHub conserva el historial y publica los archivos con GitHub Pages. GitHub Actions ejecuta las validaciones. Python 3 y Node.js se usan para desarrollar y comprobar el proyecto; no necesitás instalarlos para consultar los materiales.</p><h2>Cómo se ven los documentos</h2><p>Los originales permanecen en Google Drive. La vista previa se carga sólo cuando la pedís, mediante el visor de Drive. Esta versión no utiliza PDF.js. Moodle y Meet se abren mediante sus propios enlaces y permisos.</p><h2>Podés aportar sin programar</h2><p>Ayudanos a corregir enlaces, describir materiales o registrar nuevas fechas. En una propuesta indicá materia, unidad, período, comisión, enlace individual y fuente. Si conocés la autoría o licencia, agregá dónde figura.</p><p><a href=\"https://github.com/fran1599/materiales-tsds-ispc-1-2026-b1/issues/new/choose\" target=\"_blank\" rel=\"noopener noreferrer\">Proponer un material o una mejora en GitHub ↗</a></p><p>Las propuestas son públicas: no adjuntes trabajos personales o grupales, datos sensibles ni documentación interna. Si una corrección afecta información privada, usá el canal privado que ya tengas con el responsable.</p><h2>Si querés aportar código</h2><p>La guía explica cómo preparar el entorno, trabajar en una rama, ejecutar las comprobaciones y abrir un pull request. Cada propuesta debe explicar qué resuelve y cómo se verificó. Los aportes de código y documentación propios se incorporan bajo MIT; conservá los avisos y aportá contenido que puedas licenciar en esas condiciones.</p><p><a href=\"https://github.com/fran1599/materiales-tsds-ispc-1-2026-b1/blob/main/CONTRIBUTING.md\" target=\"_blank\" rel=\"noopener noreferrer\">Leer cómo aportar ↗</a> · <a href=\"https://github.com/fran1599/materiales-tsds-ispc-1-2026-b1/blob/main/docs/TECNICA.md\" target=\"_blank\" rel=\"noopener noreferrer\">Documentación técnica ↗</a></p><h2>Licencias y autoría</h2><div class=\"notice\"><strong>Código y documentación propios bajo MIT.</strong> Vigente desde el 18/09/2026, incluye herramientas y descripciones originales del catálogo. Los materiales educativos enlazados conservan los derechos y condiciones de sus respectivos titulares.</div><p>Que un repositorio sea público o que un archivo pueda abrirse en Drive no significa que tenga una licencia abierta. El catálogo no autoriza a modificar o redistribuir los documentos de terceros. Conservamos los enlaces originales y registramos la autoría o licencia cuando está documentada.</p><p>Podés solicitar una corrección de atribución o la revisión de un enlace por los canales de aportes. Este es un proyecto estudiantil, sin carácter oficial ni patrocinio del ISPC.</p><p><a href=\"https://github.com/fran1599/materiales-tsds-ispc-1-2026-b1/blob/main/docs/LICENCIAS.md\" target=\"_blank\" rel=\"noopener noreferrer\">Ver licencia MIT, alcance y fuentes ↗</a></p><h2>Datos y servicios externos</h2><p>La búsqueda se procesa en tu navegador. El código del sitio no incorpora analítica propia; guarda únicamente tu preferencia de tema en el navegador cuando es posible. GitHub y los servicios externos tienen sus propias políticas. No hay sincronización automática con Drive ni acceso sin conexión.</p><p><a href=\"https://github.com/fran1599/materiales-tsds-ispc-1-2026-b1\" target=\"_blank\" rel=\"noopener noreferrer\">Ver el repositorio y su historial ↗</a></p></div>";
}
function about(){
 $('#main').innerHTML=heading('SOBRE ESTE ESPACIO','Materiales que se encuentran.','Una biblioteca estudiantil para acompañar la cursada de Desarrollo de Software.')+'<div class="prose"><h2>Qué vas a encontrar</h2><p>Un catálogo de '+DATA.resources.length+' archivos: apuntes, presentaciones, prácticas, consignas generales, programas, cronogramas y documentación institucional. La colección corresponde al material disponible y revisado hasta el 19 de septiembre de 2026; no representa la totalidad de la carrera.</p><h2>Cómo se organiza</h2><p>Cada materia conserva un espacio de cursada con período, comisión y docentes cuando esos datos están documentados. Los materiales se agrupan por su contenido y por las unidades o ejes de las fuentes. Las diferencias entre nombres de carpetas, portadas y programas se señalan en las fichas o en Sobre la cursada.</p><h2>Cómo leer y descargar</h2><p>El botón Abrir lleva al archivo específico en Google Drive. Desde Ver ficha podés activar una vista previa dentro del sitio. Si Drive pide iniciar sesión o solicitar acceso, necesitás el permiso del propietario: el catálogo no modifica ni evita esos permisos. Para descargar, utilizá la opción que ofrezca Drive.</p><h2>Si se cae Moodle</h2><p>La navegación del catálogo no depende de Moodle. Los documentos siguen alojados en Drive, por lo que necesitás conexión y acceso al archivo. Este sitio no guarda una copia local de los documentos ni garantiza acceso sin internet.</p><h2>Una colección cuidada</h2><p>No se incorporan entregas personales, trabajos de grupos ni documentación interna de representación estudiantil. Los archivos que no pudieron leerse o cuya procedencia requiere confirmación permanecen fuera del catálogo. Las descripciones ayudan a identificar el contenido; no certifican su exactitud académica ni sustituyen la lectura del original.</p><h2>Cómo se actualiza</h2><p>La incorporación es revisada: agregar un archivo a Drive no lo publica automáticamente aquí. Cada nuevo material necesita descripción, materia, unidad, período y verificación de que es compartible. Las fechas y los enlaces de clases también deben tener una fuente y una comisión identificadas.</p><h2>Fechas y versiones</h2><p>La agenda muestra fechas exactas sólo cuando constan en una fuente. Cuando el cronograma indica una semana, se conserva ese intervalo. Los avisos posteriores de la materia pueden cambiar la planificación: consultá el canal oficial antes de una entrega.</p><h2>Un sitio liviano</h2><p>El catálogo usa archivos estáticos, sin cuenta propia, publicidad ni rastreadores añadidos. La vista previa carga Google Drive sólo cuando la activás. El código puede alojarse en GitHub Pages; los archivos siguen en su ubicación original.</p></div>';
}
...
function route(){
 ...
 const titles={inicio:'Biblioteca',materiales:'Materiales',agenda:'Agenda',acerca:'Sobre este espacio',proyecto:'Tecnología y aportes'};
