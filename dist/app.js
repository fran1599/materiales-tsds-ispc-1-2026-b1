...
function heading(kicker,title,desc){
 return '<div class="intro"><div><div class="eyebrow">'+esc(kicker)+'</div><h1>'+esc(title)+'</h1><p>'+esc(desc)+'</p></div><div class="review-date">Última revisión<strong>17 septiembre 2026</strong></div></div>';
}
function card(s){
 const n=count(s.id);
...
function subjectPage(id,tab='materiales'){
 ...
 if(tab==='clases'){
  ...
  $('#subject-content').innerHTML=links.length?links.map(c=>'<article class="context-box"><h2>'+esc(c.title)+'</h2><p>'+esc(c.description)+'</p><a class="open-link" target="_blank" rel="noopener noreferrer" href="'+esc(c.url)+'">Abrir clase ↗</a></article>').join(''):'<div class="empty-state"><h2>Todavía no hay enlaces de clases.</h2><p>No se incorporaron enlaces de encuentros o grabaciones verificados. Las presentaciones disponibles están en Materiales, bajo el tipo Clase.</p><a class="button secondary" href="#materia/'+id+'">Ir a materiales</a></div>';return;
 }
...
function agenda(id=null,target='#main'){
 ...
 const draw=()=>{
  ...
  $('#events').innerHTML=visible.length?visible.map(e=>{
   ...
   return '<article class="event '+state+'"><div class="event-date">'+date(e.start)+(e.end!==e.start?' – '+date(e.end):'')+'<span>'+e.start.slice(0,4)+(e.time?' · '+esc(e.time)+' h':'')+'</span><span class="status">'+label+'</span></div><div><div class="quiet" style="font-size:10px;margin-bottom:5px">'+e.subjects.map(s=>esc(subject(s).short)).join(' + ')+'</div><h3>'+esc(e.title)+'</h3><p>'+esc(e.note)+'</p><span style="font-size:11px">'+sourceLink(e.source)+'</span></div></article>';
...
function route(){
 ...
 $('#sidebar').classList.remove('open');$('#menu').setAttribute('aria-expanded','false');window.scrollTo(0,0);
}
function openResource(id){
 const r=resource(id);if(!r)return;
 $('#resource-body').innerHTML='<header class="dialog-head"><div>'+tags(r)+'<h2 id="resource-title">'+esc(r.title)+'</h2></div><button class="close-dialog" aria-label="Cerrar ficha" id="close-dialog">×</button></header><div class="dialog-content"><p>'+esc(r.description)+'</p><dl class="file-facts"><dt>Materia</dt><dd>'+r.subjects.map(s=>esc(subject(s).title)).join(' · ')+'</dd><dt>Unidad / eje</dt><dd>'+esc(r.unit)+'</dd><dt>Archivo original</dt><dd>'+esc(r.originalName)+'</dd><dt>Ubicación de origen</dt><dd>'+esc(r.sourcePath)+'</dd><dt>Revisado</dt><dd>17/09/2026 · '+esc(r.review)+'</dd><dt>Acceso</dt><dd>'+esc(r.access)+'</dd></dl><div class="dialog-buttons"><a class="button" href="'+esc(r.url)+'" target="_blank" rel="noopener noreferrer">Abrir en Drive ↗</a><button class="button secondary" id="show-preview">Ver documento aquí</button></div><div id="preview" class="preview-area"></div></div>';
...
document.addEventListener('click',e=>{if(!e.target.closest('#sidebar')&&!e.target.closest('#menu')){$('#sidebar').classList.remove('open');$('#menu').setAttribute('aria-expanded','false');}});
document.addEventListener('keydown',e=>{if(e.key==='Escape'){$('#sidebar').classList.remove('open');$('#menu').setAttribute('aria-expanded','false');}if(e.key==='/'&&!['INPUT','TEXTAREA','SELECT'].includes(document.activeElement.tagName)&&!$('#resource-dialog').open){e.preventDefault();$('#search')?.focus();}});
